...
const breadcrumbItems: BreadcrumbItem[] = [
  ...
  { title: 'Edit', href: `/pagoruan/${props.pagoruan.id}/edit` },
]
...
const form = useForm({
  kode: props.pagoruan.kode ?? '',
  nama: props.pagoruan.nama,
  provinsi_id: props.pagoruan.kecamatan?.kota_kab?.provinsi?.id ?? props.fixedKecamatan?.kota_kab?.provinsi?.id ?? null as number | null,
  kota_kab_id: props.pagoruan.kecamatan?.kota_kab?.id ?? props.fixedKecamatan?.kota_kab?.id ?? null as number | null,
  kecamatan_id: props.pagoruan.kecamatan?.id ?? props.fixedKecamatan?.id ?? null as number | null,
  alamat: props.pagoruan.alamat ?? '',
  telepon: props.pagoruan.telepon ?? '',
  nama_pimpinan: props.pagoruan.nama_pimpinan ?? '',
  nama_pelatih: props.pagoruan.nama_pelatih ?? '',
  is_active: props.pagoruan.is_active,
})
...
function submit() {
  form.put(`/pagoruan/${props.pagoruan.id}`)
}
</script>

<template>
  <AppLayout :breadcrumbs="breadcrumbItems">
    <Head title="Edit Pagoruan" />

    <div class="mx-auto max-w-4xl space-y-6 p-6">
      <div>
        <h1 class="text-3xl font-semibold text-slate-950 dark:text-white">Edit Pagoruan</h1>
        <p class="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-300">
          Perbarui data pagoruan dan pastikan kecamatannya tetap sesuai.
        </p>
      </div>
      ...
      >
        Akses Anda dibatasi ke kecamatan
